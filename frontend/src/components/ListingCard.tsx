        ...
        >
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                    {posting.research_areas && (
                        <div>
                            ...
                            <p className="text-gray-600">{posting.research_areas}</p>
                        </div>
                    )}
                    {posting.time_commitment && (
                        <div>
                            <span className="font-medium text-gray-700">Time Commitment:</span>
                            <p className="text-gray-600">{posting.time_commitment}</p>
                        </div>
                    )}
                </div>
            ) : (
                <div className="space-y-4 mb-4 text-sm">
                    ...
                    {posting.requirements && (
                        <div>
                            ...
                            <p className="text-gray-600">{posting.requirements}</p>
                        </div>
                    )}
                    {posting.time_commitment && (
                        <div>
                    ...
                    {posting.compensation_details && (
                        <div>
                            ...
                            <p className="text-gray-600">{posting.compensation_details}</p>
                        </div>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <span className="font-medium text-gray-700">Posted:</span>
                            <p className="text-gray-600">{new Date(posting.posted_at).toLocaleDateString()}</p>
                        </div>
                        <div>
                            <span className="font-medium text-gray-700">Faculty ID:</span>
                            <p className="text-gray-600">{posting.faculty_id}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {posting.application_deadline && (
                <div className="flex items-center gap-4 text-xs text-gray-500 pt-3 border-t border-gray-100">
                    <span>
                        <span className="font-medium">Deadline:</span> {new Date(posting.application_deadline).toLocaleDateString()}
                    </span>
                    {posting.start_date && (
                        <span>
                            <span className="font-medium">Start:</span> {new Date(posting.start_date).toLocaleDateString()}
                        </span>
                    )}
                </div>
            )}
